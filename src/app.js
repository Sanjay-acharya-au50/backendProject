...
// ----------------------------------------------------
    ...
    //     const user_data = await userClass.findOne({email:user_email})
    //     if(!user_data){
    //         res.status(400);
    ...
    //     // to string because of bcrypt unable to convert string because its external package
    ...
    //     }
    //     // jwt token
    //     const jwt_token = jwt.sign({id: user_data._id}, "mysecretkey", {expiresIn: "1h"});
    //     // its a key value pair, 1st one is key, 2nd is value that we want to store
    //     //  token_name = token name
    //     //  jwt_token is variable of 78
    //     res.cookie("token_name", jwt_token);
...
// ----------------------------------------------------
// in the url dont show the entered email and password
// app.post("/user_signup", async (req,res)=>{

//     const data = req.body;
//     if(data.password != data.cpassword){
//        return res.send("wrong confirm password");
//     }
...
//     //     res.status(404);
//     //    return res.send("feilds are emply");
//     // }
//     // if already has a account
//     const user_data = await userClass.findOne({email : user_email});
//     if(user_data){
//         res.status(400);
//        return res.send("user already exist");
//     }
//     // 10 times recursive and generate random encoded password
//     const salt = await bcrypt.genSalt(10);
//     // in the url dont show the entered email and password
//     let hashed_password = await bcrypt.hash(user_password.toString(), salt);
//     let user_data_store = new userClass({name: user_name, email:user_email, password:hashed_password});
//     const result = await user_data_store.save();
//     console.log(result);
